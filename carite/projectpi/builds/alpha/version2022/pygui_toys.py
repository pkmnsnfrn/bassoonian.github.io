...
def make_window(theme):
    ...
    layout = [
                [sg.Text('Demo Of (Almost) All Elements', justification='center', font=("Helvetica", 16), relief=sg.RELIEF_RIDGE, k='-TEXT HEADING-', enable_events=True)]]
    layout +=[[sg.TabGroup([[  sg.Tab('General', input_layout),
                               sg.Tab('Lexicon', asthetic_layout),
                               sg.Tab('Sound Changes', graphing_layout),
                               sg.Tab('Morphology', popup_layout),
                               sg.Tab('Theming', theme_layout),
                               sg.Tab('Output', logging_layout)]], key='-TAB GROUP-', expand_x=True, expand_y=True),

               ]]
    layout[-1].append(sg.Sizegrip())
    window = sg.Window('Project π', layout, right_click_menu=right_click_menu_def, right_click_menu_tearoff=True, grab_anywhere=True, resizable=True, margins=(0,0), use_custom_titlebar=True, finalize=True, keep_on_top=True)
...
if __name__ == '__main__':
    ...
    st1 = "bhéttrom"
    st2 = "bhéthtrom"
    s = difflib.SequenceMatcher(None, st1, st2)
    pat = []
    for block in s.get_matching_blocks():
        print(block)
        if block[2] != 0:
            pat.append([block[1], block[1] + block[2]])
    res = ""
    index = 0
    while len(pat) > 0:
        while index < pat[0][0]:
